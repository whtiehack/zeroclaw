...
fn llm_user_content_with_sender_identity(msg: &traits::ChannelMessage, content: &str) -> String {
    ...
    let prefix = if msg.channel == "wecom" {
        format!("[sender_userid={sender}]")
    } else {
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    #[test]
    fn runtime_defaults_snapshot_falls_back_to_context_skills() {
        let mut skills = crate::config::SkillsConfig::default();
        skills.allow_scripts = true;
        skills.prompt_injection_mode = crate::config::SkillsPromptInjectionMode::Full;
        skills.trusted_skill_roots = vec!["/tmp/shared-skills".to_string()];
        ...
        let defaults = runtime_defaults_snapshot(&runtime_ctx);
        assert_eq!(defaults.skills.allow_scripts, skills.allow_scripts);
        assert_eq!(
            defaults.skills.prompt_injection_mode,
            skills.prompt_injection_mode
        );
        assert_eq!(
            defaults.skills.trusted_skill_roots,
            skills.trusted_skill_roots
        );
    }
