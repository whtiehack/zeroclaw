...
impl Tool for CronAddTool {
    ...
    fn description(&self) -> &str {
        "Create a scheduled cron job (shell or agent) with cron/at/every schedules. \
         ...
         To deliver output to a channel (Discord, Telegram, Slack, Mattermost, QQ, Napcat, WeCom, Lark, Feishu, Email), set \
            ...
            "properties": {
                ...
                "delivery": {
                    ...
                    "properties": {
                        ...
                        "channel": { "type": "string", "enum": ["telegram", "discord", "slack", "mattermost", "qq", "napcat", "wecom", "lark", "feishu", "email"], "description": "Channel type to deliver to" },
...
mod tests {
    ...
    #[test]
    fn schema_includes_wecom_delivery_channel() {
        let cfg = Arc::new(Config::default());
        let tool = CronAddTool::new(cfg.clone(), test_security(&cfg));
        let schema = tool.parameters_schema();
        let channels = schema["properties"]["delivery"]["properties"]["channel"]["enum"]
            .as_array()
            .expect("delivery.channel enum should be an array");
        assert!(channels.iter().any(|value| value.as_str() == Some("wecom")));
    }
