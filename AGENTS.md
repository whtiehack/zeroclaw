...
## 0.1) Clean Workspace Gate (Mandatory)

- Do not perform substantive task work in a dirty workspace.
- If the current location is dirty, stash or commit unrelated changes before starting a new task.
- Create a **dedicated git worktree** only for new feature development or major/multi-file changes; small fixes, docs edits, and routine maintenance can work directly on a clean branch without a worktree.
